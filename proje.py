...
users = {
    ...
    "Alireza12CV": {
        "password": "2277020CVVG",
        "role": "employee",
        "fullname": "Ali Jamshidi",
        "wallet": 0,
        "history": [],
        "email": "",
        "phone": "",
    ...
    "Amini567": {
        "password": "20942XsvH@Z",
        "role": "customer",
        "fullname": "Ramin Amini",
        "wallet": 0,
        "history": [],
        "email": "",
        "phone": "",
...
discounts = ["QWBv23@#v.3", "sd67Hj#!x.2", "23FCV%n&5"]
...
def customer_menu(username):
    ...
    while True:
        ...
        if choice == "1":
            ...
            action = input(
                "آیا می‌خواهید غذایی به سبد خرید اضافه کنید یا حذف کنید؟ (add/remove): "
            ).lower()
            if action == "add":
            ...
            if username in users and "wallet" in users[username]:
                ...
                if wallet >= total:
                    ...
                    print(
                        f"خرید شما با موفقیت انجام شد. مبلغ {total} تومان از کیف پول شما کسر شد."
                    )
...
def employee_menu(username):
    while True:
        ...
        if action == "1":
            ...
            change_field = input(
                "کدام فیلد را می‌خواهید تغییر دهید؟ (ایمیل/شماره تلفن/غذای مورد علاقه): "
            )
...
def manager_menu(username):
    while True:
        ...
        if action == "1":
            ...
            if (
                employee_username in users
                and users[employee_username]["role"] == "employee"
            ):
                change_field = input(
                    "کدام فیلد را می‌خواهید تغییر دهید؟ (ایمیل/شماره تلفن/غذای مورد علاقه): "
                )
                ...
                print(f"کاربر وارد شده کارمند نیست یا وجود ندارد.")

        elif action == "4":
            # تغییر اطلاعات مشتریان
            customer_username = input("نام کاربری مشتری را وارد کنید: ")
            if (
                customer_username in users
                and users[customer_username]["role"] == "customer"
            ):
                change_field = input(
                    "کدام فیلد را می‌خواهید تغییر دهید؟ (ایمیل/شماره تلفن/غذای مورد علاقه): "
                )
...
def search_user():
    print("1. جست و جو بر اساس نام کاربری")
    print("2. جست و جو بر اساس آدرس ایمیل")
    print("3. جست و جو بر اساس شماره همراه")
    print("4. بازگشت")
    ...
    if choice == "1":
        ...
        if username in users:
            print("اطلاعات کاربر:", users[username])
        else:
            print("کاربر یافت نشد.")

    elif choice == "2":
        email = input("آدرس ایمیل را وارد کنید: ")
        found = False
        for user in users.values():
            if user.get("email") == email:
                print("اطلاعات کاربر:", user)
                found = True
                break
        if not found:
            print("کاربر با این آدرس ایمیل یافت نشد.")

    elif choice == "3":
        phone = input("شماره همراه را وارد کنید: ")
        found = False
        for user in users.values():
            if user.get("phone") == phone:
                ...
                break
        if not found:
            print("کاربر با این شماره همراه یافت نشد.")
